...
- Add `VerticalScrollColumn`, `HorizontalScrollRow`, `VerticalScrollBox`, and `HorizontalScrollBox` scrolling layout composables in the `layout.ext` package as replacements for the scroll modifiers.
...
- Add the badge composable `BadgedBox` and the `Badge` composable in `ext` (#70, #72).
- Add the chip composables `AssistChip`, `FilterChip`, `InputChip`, `SuggestionChip` (and elevated variants) in `ext` (#70, #72).
- Add the dialog composables `AlertDialog` and `SimpleDialog` (#70, #72).
- Add the divider composable `HorizontalDivider` (#70, #72).
- Add the navigation drawer composable `ModalNavigationDrawer` (deprecated upon review) (#70, #72).
- Add the radio button composables `RadioButton`, and `RadioButtonRow` and `radioGroup` in `ext` (#70, #72).
- Add the segmented button composables `SingleChoiceSegmentedButtonRow`, `MultiChoiceSegmentedButtonRow`, and scoped `SegmentedButton` in `ext` (#70, #72).
- Add the select composables `FilledSelect`, `OutlinedSelect`, and `SelectOption` in `ext` (#70, #72).
- Add the slider composables `Slider` and `RangeSlider` (#70, #72).
- Add the snackbar composables `SnackbarHost`, `SnackbarHostState`, and related types (`SnackbarVisuals`, `SnackbarData`, `SnackbarResult`, `SnackbarDuration`) (#91, #98).
- Add the tab composables `PrimaryTabRow`, `SecondaryTabRow`, and `PrimaryTab`, `SecondaryTab` in `ext` (#70, #72).
...
- Save ViewModel state in navigation on JS DOM (#55).
...
- Support ES2015 compilation target by replacing `require()` with ES module imports.
...
- Deprecate the `Modifier.verticalScroll` and `Modifier.horizontalScroll` modifiers for external use. Use `VerticalScrollBox`/`VerticalScrollColumn` and `HorizontalScrollBox`/`HorizontalScrollRow` instead.
- Deprecate `ModalNavigationDrawer` upon review.
...
- Fix snackbar not showing on JS DOM and fix clicking the close button preventing subsequent snackbar appearances (#98).
- Fix top app bar scrollbar overflow with `box-sizing: border-box` (#109).
- Fix `Scaffold` snackbar positioning to dynamically measure FAB height (#110).
- Fix `RadioButtonRow` visual inconsistency caused by `conventionalPadding` set on the wrong composable.
- Fix `*Select` state sync on JS DOM via `value`-based selection (#81).
- Fix the issue that `withAndroidTarget` was put in the wrong `group` in `applyDefaultHierarchyTemplate`.
...
- Bump Dokka to 2.1.0.
- Enable `kotlin.native.enableKlibsCrossCompilation`.
